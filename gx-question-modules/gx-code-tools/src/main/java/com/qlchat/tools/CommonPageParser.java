...
public class CommonPageParser {
	...
	static{
		try{
			//获取文件模板根路径
			String  templateBasePath = CreateJava.getRootPath()+"src/main/template" ;
	...
	public static void WriterPageDoc(VelocityContext context,String templateName,String fileDirPath,String targetFile){
		try{
			File file = new File(fileDirPath+targetFile);
			if(!file.exists()){
				new File(file.getParent()).mkdirs();
